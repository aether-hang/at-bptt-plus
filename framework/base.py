...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = Distill(
        ...
        cctype=args.cctype,
        total_unroll_steps=args.totwindow,
        window_radius=args.window_radius,
        selector_kl=args.selector_kl,
        selector_ref_ema=args.selector_ref_ema,
        selector_tau_w=args.selector_tau_w,
        selector_eps=args.eps,
        use_cr_lrha=args.use_cr_lrha,
        cr_rank=args.cr_rank,
    ...
    save_dir = './save/{}'.format(str(args.dataset))
    os.makedirs(save_dir, exist_ok=True)
    fname = (
        './save/{}/{}_IPC_{}_window_{}_traj_{}_{}.h5'.format(
            str(args.dataset),
            str(args.ddtype),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch - args.start_epoch + start_test_epoch) % args.test_freq == 0:
            ...
            if model.module.data.weight.get_device() == 0 and args.wandb:
                latest_grad_norm = grad_tmp[-1] if len(grad_tmp) > 0 else 0.0
                wandb.log(
                    {
                        "loss": test_loss,
                        "epoch": int(epoch),
                        ...
                        "selected_trunc_idx": model.module.last_truncation_index,
                        "selected_window": model.module.last_window_size,
                        "selector_beta": model.module.last_beta,
                        "cr_refreshed": int(model.module.last_cr_refreshed),
                    }
                )
        ...
        # remember best acc@1 and save checkpoint
            ...
            # update curriculum for legacy mode only
...
def train(train_loader, model, criterion, optimizer, epoch, device, distill_steps, args):
    ...
    use_legacy_random_curriculum = args.ddtype == 'curriculum' and model.module.cctype == 2
    if use_legacy_random_curriculum:
        if args.rank == 0:
    ...
    for i, (inputs, targets) in enumerate(train_loader):
        if args.complete_random and use_legacy_random_curriculum:
            if args.rank == 0:
                shared_curriculum = torch.tensor(random.randint(args.minwindow, args.totwindow-args.window))
            else:
                shared_curriculum = torch.tensor(0)
...
def calculate_grad_norm(grad_norm):
    valid = grad_norm[grad_norm > 1e-5]
    if valid.numel() == 0:
        return 0.0
